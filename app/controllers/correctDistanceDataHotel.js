...
exports.correctDistanceData = function(){
    return new Promise(function (resolve, reject) {			
        mysql_connection_hotel.query('SELECT Resort_ID FROM ' + config.mysqlData.rawData.HOTEL_TABLES.RAW_DATA_COLLECTION_SUCCESS + ' WHERE not success = "not processed" limit 100' + config.mysql.MYSQL_DELIMITER, function(err, rows, fields){
            if(err){    
                reject(err);
            }
            console.log(rows[0]['Resort_ID']);
            for(var i = 0; i < rows.length; i++){
                var query = solrClientHotel.createQuery()
				.q("id:" + rows[i]['Resort_ID'])
				.start(0)
				.rows(5)
				.set("sort=id%20asc")
				;
